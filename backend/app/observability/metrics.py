...
- API-side ingestion metrics live in `app.observability.api_metrics`.
- This module is worker/task-side only and is exposed via `app.observability.worker_metrics_exporter`.
...
Multiprocess Mode (B0.5.6.5: worker/exporter):
    For pre-forked Celery workers, set
    ...
    and the worker metrics exporter aggregates them via MultiProcessCollector.
    
    Example:
        export PROMETHEUS_MULTIPROC_DIR=/tmp/prometheus_multiproc
        # Directory must already exist and be writable (provisioned externally).
        celery -A app.celery_app.celery_app worker -P prefork -c 4
    
    Without PROMETHEUS_MULTIPROC_DIR, metrics are process-local to the worker process.
"""
