...
def _publish_tenant_policy(control: dict[str, str], state: str) -> None:
    """Use the supported operator command in an isolated production-image job."""
    image = _compose("images", "-q", "api").stdout.strip().splitlines()[0]
    subprocess.run([
        "docker", "run", "--rm", "--network", _required("C19_COMPOSE_PROJECT_NAME") + "_default",
        "-e", "TRUST_POLICY_ADMIN_DATABASE_URL=postgresql://app_trust_policy_admin:app_trust_policy_admin@postgres:5432/skeldir_c19",
        image, "python", "-m", "app.trust.policy_configuration",
        "--tenant-id", control["tenant_id"], "--policy-state", state,
        "--approval-reference", "c19-explicit-design-partner-configuration",
...
def test_c19_context_robust_production_topology() -> None:
    ...
    _publish_tenant_policy(control_a, "simulation_only")
    ...
    # VII: final-artifact mapping and supported consumer conduction are mandatory.
    with _admin_connection() as conn, conn.cursor() as cur:
        cur.execute("SELECT set_config('app.current_tenant_id',%s,true)", (control_a["tenant_id"],))
        cur.execute("SELECT envelope_hash,final_envelope_hash,semantic_truth_hash FROM public.trust_final_issuance_identity WHERE tenant_id=%s AND source_envelope_id=%s", (control_a["tenant_id"], envelope["envelope_id"]))
        mappings = cur.fetchall()
    assert len(mappings) == 1
    assert mappings[0][1] == compute_envelope_payload_hash(envelope)
    assert mappings[0][2] == envelope["semantic_truth_hash"]
    p14_source_binding = {"issuance_log_rows": 1, "matched_by_envelope_hash": 1,
        "matched_by_semantic_truth": 1, "audit_stage_hash": mappings[0][0],
        "final_envelope_hash": mappings[0][1], "final_semantic_truth_hash": mappings[0][2]}
    conducted_policy_state = envelope["policy_action_authority"]["policy_state"]
    assert conducted_policy_state == "simulation_only"
    submitted = httpx.post(f"{base_url}/api/trust/v1/simulations",
        headers=_machine_headers(control_a, prefix="p14-positive"),
        json={"source_envelope_id": envelope["envelope_id"], "source_final_envelope_hash": compute_envelope_payload_hash(envelope), "total_budget_minor": 10000,
              "request_ref": f"req_{uuid4().hex}"}, timeout=API_TIMEOUT_SECONDS)
    assert submitted.status_code == 201, submitted.text
    assert submitted.json()["status"] == "completed", submitted.text
    request_id = submitted.json()["request_id"]
    readback = httpx.get(f"{base_url}/api/trust/v1/simulations/{request_id}",
        headers=_machine_headers(control_a, prefix="p14-read"), timeout=API_TIMEOUT_SECONDS)
    assert readback.status_code == 200, readback.text
    artifact = readback.json()
    assert artifact["source_envelope_id"] == envelope["envelope_id"]
    assert artifact["source_semantic_truth_hash"] == envelope["semantic_truth_hash"]
    assert sum(x["allocation_minor"] for x in artifact["allocations"]) == 10000
    assert artifact["requires_human_approval"] is True
    assert artifact["causal_authority"] == artifact["external_execution_authority"] == "none"
    foreign_read = httpx.get(f"{base_url}/api/trust/v1/simulations/{request_id}",
        headers=_machine_headers(control_b, prefix="p14-foreign-read"), timeout=API_TIMEOUT_SECONDS)
    assert foreign_read.status_code == 404, foreign_read.text
    p14_outcome = {"outcome": "CONDUCTED", "identifiers": {"persisted": "true", "request_id": request_id}}
    conducted_channels = artifact["allocations"]
    census_sql = """SELECT
        (SELECT count(*) FROM public.b28_simulation_requests WHERE tenant_id=%s),
        (SELECT count(*) FROM public.b28_simulation_results WHERE tenant_id=%s),
        (SELECT count(*) FROM public.b28_proposals WHERE tenant_id=%s),
        (SELECT count(*) FROM public.b28_request_authentications WHERE tenant_id=%s)"""
    p14_side_effects = _fetch_one(census_sql, (control_a["tenant_id"],)*4)
    assert tuple(p14_side_effects) == (1,1,1,1)
    durable_requests,durable_results,durable_proposals,durable_witnesses = p14_side_effects
    _publish_tenant_policy(control_a, "read_only")
    negative_trust = httpx.get(f"{base_url}/api/trust/v1/envelopes/confidence_projection/{subject_ref}",
        headers=_machine_headers(control_a, prefix="p14-negative-trust"), timeout=API_TIMEOUT_SECONDS)
    assert negative_trust.status_code == 200, negative_trust.text
    assert negative_trust.json()["policy_action_authority"]["policy_state"] == "read_only"
    for source_envelope in (negative_trust.json(), envelope):
        refused = httpx.post(f"{base_url}/api/trust/v1/simulations",
            headers=_machine_headers(control_a, prefix="p14-negative"),
            json={"source_envelope_id": source_envelope["envelope_id"], "source_final_envelope_hash": compute_envelope_payload_hash(source_envelope), "total_budget_minor":10000,
                  "request_ref":f"req_{uuid4().hex}"}, timeout=API_TIMEOUT_SECONDS)
        assert refused.status_code == 403, refused.text
    assert _fetch_one(census_sql, (control_a["tenant_id"],)*4) == p14_side_effects
    print("P14_VII_POSITIVE_READ_AND_NEGATIVE_CONSERVATION_PASS")
    ...
    # Printed, not only recorded: the evidence JSON lands in an ephemeral runtime
    # directory the workflow does not upload, and these are the facts an auditor
    # reading the log needs about the P14 boundary.
    print(f"c19_p14_boundary_outcome={p14_outcome['outcome']}")
    print(
        "c19_p14_refusing_conjunct="
        f"{p14_source_binding.get('refusing_conjunct', '-')}"
    )
