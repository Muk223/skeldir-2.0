#!/usr/bin/env python3
"""Shared producer-side evidence-cell construction for B2.6-P1."""
...
def migration_head() -> str:
    ...
    if len(heads) != 1:
        raise EvidenceError(f"b26_p1_expected_one_migration_head:{sorted(heads)}")
...
def build_evidence_cell(
    *,
    ...
    if expected_sha != sha:
        raise EvidenceError(
            f"b26_p1_checkout_candidate_mismatch:expected={expected_sha}:actual={sha}"
        )
