...
# Mechanism: Container internal network + iptables egress DROP
# Usage: Source this before running evaluation harness
#
# HARD REQUIREMENT: This must be ACTIVE during authoritative runs
#
# Required env:
#   CONTAINER_RUNTIME: CLI used to manage containers (e.g., podman)
#   CONTAINER_BRIDGE: bridge interface name (optional; defaults to cni0)

set -euo pipefail

if [ -z "${CONTAINER_RUNTIME:-}" ]; then
  echo "CONTAINER_RUNTIME env var must be set to a container CLI" >&2
  exit 1
fi

CONTAINER_BRIDGE="${CONTAINER_BRIDGE:-cni0}"
...
# ============================================================================
# Step 1: Container Network Isolation
# ============================================================================

# Create isolated internal network if not exists
$CONTAINER_RUNTIME network ls | grep -q skeldir-r0-isolated || \
  $CONTAINER_RUNTIME network create --internal skeldir-r0-isolated

echo "ƒo" Container internal network 'skeldir-r0-isolated' created/verified"
...
if command -v iptables &> /dev/null; then
  ...
  # (Adjust CONTAINER_BRIDGE based on your setup)
  ...
  echo "ƒo" iptables egress DROP rules applied"
  echo "  Rules captured to: $ARTIFACTS_DIR/iptables_rules.txt"
else
  echo "ƒsÿ iptables not available (Windows or non-Linux)"
  echo "  Network isolation will rely on the container internal network only"
...
$CONTAINER_RUNTIME run --rm --network skeldir-r0-isolated alpine:3.18 \
...
if [ "$PROBE_RESULT" = "FAIL_ISOLATION_BROKEN" ]; then
  echo "ƒ?O CRITICAL: Network isolation is NOT enforced!"
  echo "   Container was able to reach public internet"
  echo "   EG-R0-6 FAILS - cannot proceed with authoritative run"
  exit 1
else
  echo "ƒo" Network isolation verified: Public egress blocked"
fi
...
1. Container internal network: skeldir-r0-isolated (ENABLED)
...
Status: $([ "$PROBE_RESULT" = "PASS_EGRESS_BLOCKED" ] && echo "ƒo" ISOLATED" || echo "ƒ?O FAIL")
