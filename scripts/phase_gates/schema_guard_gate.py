#!/usr/bin/env python3
"""
Schema contract guard gate runner.
"""
...
def main() -> int:
    ...
    if "DATABASE_URL" not in env:
        ...
        return 1
    summary_path = EVIDENCE_DIR / "schema_guard_summary.json"
    try:
        run(["alembic", "upgrade", "202511131121"], "schema_guard_alembic_core.log", env=env)
        run(["alembic", "upgrade", "skeldir_foundation@head"], "schema_guard_alembic_foundation.log", env=env)
        run(
            ["python", "-m", "pytest", "backend/tests/test_schema_contract_guard.py", "backend/tests/test_no_raw_inserts_core_tables.py", "-q"],
            "schema_guard_pytest.log",
            env=env,
        )
        summary = {
            "phase": "SCHEMA_GUARD",
