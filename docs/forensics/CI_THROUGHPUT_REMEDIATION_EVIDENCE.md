...
- **LANDED**: PR #715 merged as `93dbdb14` (2026-09-07T00:03:45Z) via `main-merge-queue` ALLGREEN, no bypass (C-01 ✓). Fourth queue entry: 21-run burst, all completed; fixed `b11-p4-ci-audit-gate` reported success on the speculative commit.
- **Steady-state docs-only (post-merge PR #719, README-only): 24 runs vs 42 pre (−43%)** — 22 `pull_request` (21 required workflows + exempt empirical) + 2 `pull_request_target` (required b11-p2/p4 fork doubles); advisory b11-p1/p3 correctly absent. Gate 1/3 live ✓.
- PR #715 CI queue numbers (run 34056154235: wall 41.55, queue med 24.03) are session-confounded (3 PRs + reruns in one hour; min-queue 6.32 vs 0.02 proves backlog at start) and are recorded as directional only. The controlled evidence is the validated fan-out table (−31…−42% PR, −60% MG) against the documented 20-slot quota.
